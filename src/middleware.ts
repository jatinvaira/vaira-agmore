...
export default authMiddleware({
    ...
    // const pathWithSearchParams = `${url.pathname}${searchParams.length>0?`?${searchParams}`:`fds`}`
    ...
    const customSubDomain = hostname ? hostname.split(process.env.NEXT_PUBLIC_DOMAIN).filter(Boolean)[0] : null
