        ...
        <Form {...form}>
          ...
          >
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="w-[400px] p-2 my-1 bg-slate-900 rounded-lg shadow-lg border border-gray-800">
                <div className="p-2 rounded-lg mb-2">
                  <input
                    placeholder="Search..."
                    className="w-full h-9 px-3 py-2 text-sm border rounded-md focus:outline-none focus:border-indigo-600"
                    value={search}
                    onChange={(e) => {
                      ...
                      if (saveTimerRef.current)
                        clearTimeout(saveTimerRef.current);
                      saveTimerRef.current = setTimeout(async () => {
                        const response = await searchContacts(e.target.value);
                        setContactList(response);
                      }, 1000);
                    }}
                  />
                </div>
                <DropdownMenuSeparator className="my-2 border-t border-gray-800" />

                {contactList?.length > 0 ? (
                  contactList.map((c) => (
                    <DropdownMenuItem
                      key={c.id}
                      onSelect={() => setContact(c.id === contact ? "" : c.id)}
                      className="flex items-center justify-between px-2 py-1 rounded-md hover:bg-gray-800"
                    >
                      {c.name}
                      {contact === c.id && (
                        <CheckIcon className="ml-auto h-4 w-4 text-indigo-500 opacity-100" />
                      )}
                    </DropdownMenuItem>
                  ))
                ) : (
                  <DropdownMenuLabel className="px-2 py-1 text-gray-500">
                    No Customer found.
                  </DropdownMenuLabel>
                )}
              </DropdownMenuContent>
